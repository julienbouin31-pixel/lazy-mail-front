  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div v-if="user?.cancelAtPeriodEnd && user?.subscriptionEndDate" class="bg-amber-500 text-white">
      <div class="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span class="text-sm font-medium">{{ $t('dashboard.cancelBanner', { date: formatDate(user.subscriptionEndDate) }) }}</span>
        </div>
    ...
    <div v-else-if="user?.plan !== 'pro'" class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white">
      <div class="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span class="text-sm font-medium">{{ $t('dashboard.upgradeBanner') }}</span>
        </div>
    ...
    <div v-if="showSelectModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl">
        <h2 class="text-lg font-bold mb-2">{{ $t('dashboard.selectModal.title') }}</h2>
        <p class="text-sm text-gray-500 mb-6">{{ $t('dashboard.selectModal.description') }}</p>
        ...
        <NuxtLink to="/pricing" class="block text-center text-sm text-purple-600 font-medium hover:underline">
          {{ $t('dashboard.selectModal.unlock') }}
        </NuxtLink>
      </div>
    </div>
    ...
    <main class="max-w-6xl mx-auto px-6 py-8">
      <div class="flex justify-between items-end mb-8">
        <div>
          ...
          <p class="text-gray-500 text-sm">{{ $t('dashboard.description') }}</p>
        </div>
      </div>

      <!-- Style Générique -->
      <div v-if="genericContact" class="mb-8">
        <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{{ $t('dashboard.genericStyle') }}</h2>
        <NuxtLink
        ...
        >
          ...
          <h3 class="font-bold text-lg mb-1">{{ $t('dashboard.genericStyle') }}</h3>
          <p class="text-sm text-gray-500">{{ $t('dashboard.genericStyleDescription') }}</p>
        </NuxtLink>
      </div>

      <div v-if="pending" class="text-center py-20 text-gray-400">{{ $t('dashboard.loading') }}</div>
      ...
      <div v-else-if="!contacts?.length && !genericContact" class="text-center py-20">
        ...
        <h3 class="text-lg font-semibold mb-2">{{ $t('dashboard.noContacts') }}</h3>
        <p class="text-gray-500 text-sm max-w-md mx-auto mb-6">
          {{ $t('dashboard.noContactsDescription') }}
        </p>
        <a href="#" class="inline-flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-lg font-medium hover:bg-gray-800 transition">
          ...
          {{ $t('dashboard.downloadExtension') }}
        </a>
      </div>

      <h2 v-if="contacts?.length" class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{{ $t('dashboard.contacts') }}</h2>
        ...
        >
          <div class="flex justify-between items-start mb-4">
            ...
            <div class="text-right">
              ...
              <span v-else class="text-xs font-medium bg-gray-100 px-2 py-1 rounded text-gray-600">{{ $t('dashboard.configure') }}</span>
            </div>
          </div>
...
function formatDate(date) {
  return new Date(date).toLocaleDateString(locale.value === 'fr' ? 'fr-FR' : 'en-US', { day: 'numeric', month: 'long', year: 'numeric' })
}
</script>
